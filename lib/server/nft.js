    ...
    const result = [];
     ...
     for (let tx of txs) {
      ...
      const r = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_HODL_API_ADDRESS}/profile/nickname?address=${tx.args.seller}`), 
        fetch(`${process.env.NEXT_PUBLIC_HODL_API_ADDRESS}/profile/nickname?address=${tx.args.buyer}`)]
      );
